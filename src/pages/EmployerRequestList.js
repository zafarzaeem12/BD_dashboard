...
import { approveDisapprove, blockUnblock, deleteAccount, getAllUsers, getEmployersRequest, getUserStatus } from '../store/slices/userSlice';
...
const EmployerRequestList = () => {
    ...
    users?.map((item) =>
        csvData.push([`${item?.user_name}`, `${item?.user_email}`, `${item?.user_phone}`, `${item?.state}`, `${item?.user_is_verified}`, `${item?.zipCode}`, `${item?.role}`])
    )
    ...
    const approveDisapproveEmployer = async (id, approve) => {
        try {
            await dispatch(approveDisapprove(id, approve)).unwrap()
            $('#tableData')
                .DataTable().destroy();
            try {
                Users()
    ...
    const Users = async () => {
        try {
            setUsers(null)
            const response = await dispatch(getEmployersRequest()).unwrap()
            ...
            >
                <div className='change-password-modal' id="exampleModalCenter" tabIndex={-1} aria-labelledby="exampleModalCenterTitle" aria-hidden="true" style={{ display: "block", zIndex: 100 }}>
                    {modalType == "userDetail" ? <>
                        <p className="pass-text">Employer Request Detail</p>
                                            ...
                                            <table id="tableData" className="table table-bordered" style={{ width: '100%', textAlign: "center" }}>
                                                        ...
                                                        <tr key={i}>
                                                            ...
                                                            <td>
                                                                <span className="edit-icon" >
                                                                    ...
                                                                    <span style={{ cursor: "pointer", fontWeight: "bold", margin: 10 }} onClick={() => approveDisapproveEmployer(item?._id)}  ><i className="fa fa-solid fa-ban"> Approve</i></span>
                                                                    {/* <span style={{ cursor: "pointer", fontWeight: "bold", margin: 10 }} onClick={() => approveDisapproveEmployer(item?._id)}  ><i className="fa fa-unlock-alt"> DisApprove</i> </span> */}
                                                                </span>
                                                            </td>
                                                        </tr>
                                                    )
                                                ...
                                                <tfoot>
                                                    {users ? (<tr>
                                                        ...
                                                        <th>Detail</th>
                                                        <th>Action</th> */}
                                                    </tr>) : (<tr><th>{status == "loading" ? "Loading..." : "No Request Found"}</th></tr>)}
                                                </tfoot>
                                            </table>
                                        </div>
                                    </div>
